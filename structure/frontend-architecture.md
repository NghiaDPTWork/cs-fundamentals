...
2.  **Tích hợp sẵn các Engine biên dịch chuyên biệt**:
    ...
    *   *Tại sao không có Python hay Java?* Câu hỏi này có cả câu trả lời về lịch sử và kỹ thuật:
        *   **Bối cảnh lịch sử (1995)**: Khi Netscape Navigator (trình duyệt phổ biến nhất bấy giờ) muốn tạo một ngôn ngữ kịch bản cho web, họ muốn một thứ gì đó "nhẹ nhàng, dễ học" dành cho các nhà thiết kế web phi lập trình để làm các tương tác nhỏ (như validate form). Java thời điểm đó (của Sun Microsystems) quá nặng nề và phức tạp. Brendan Eich đã được giao nhiệm vụ thiết kế một ngôn ngữ kịch bản gọn nhẹ và ông đã tạo ra **Mocha** (sau đó đổi tên thành **JavaScript**) chỉ trong vỏn vẹn **10 ngày**.
        *   **Kỷ nguyên Java Applets thất bại**: Thực tế, Java đã từng chạy trên trình duyệt qua **Java Applets**. Tuy nhiên, nó đã thất bại do: (1) *Hiệu năng*: Load applet yêu cầu khởi chạy cả cỗ máy ảo JVM khổng lồ gây đơ lag; (2) *Khả năng tích hợp*: Applet chạy cô lập như một plugin ngoài, không thể trực tiếp tương tác hay chỉnh sửa cây DOM/CSSOM của trang web một cách mượt mà như JS; (3) *Bảo mật*: Java Applets liên tục bị khai thác lỗi bảo mật hệ thống nghiêm trọng, dẫn đến việc các trình duyệt khai tử hoàn toàn công nghệ này.
        *   **Lý do không dùng Python**: Vào năm 1995, Python vẫn đang ở phiên bản sơ khai (1.x) và chưa phổ biến. Khi JavaScript đã có lợi thế đi trước (First-mover Advantage) và được chuẩn hóa toàn cầu thông qua **ECMAScript** (1996), việc thuyết phục các đối thủ cạnh tranh khốc liệt (Microsoft, Google, Apple) cùng đồng thuận tích hợp thêm một runtime Python vào trình duyệt của họ là điều bất khả thi về mặt thương mại lẫn kỹ thuật.
        *   **Bypass hiện đại**: Ngày nay, nếu muốn chạy Python, Rust hay C++ trên web, lập trình viên có thể biên dịch chúng sang **WebAssembly (Wasm)** để chạy song song với JS ở tốc độ gần như native. Tuy nhiên, Wasm vẫn phải đi qua JS nếu muốn tương tác trực tiếp với DOM.
