...
> [!IMPORTANT]
> **Phân biệt `process.nextTick()` vs `setImmediate()`:**
> *   `process.nextTick()` chạy **ngay lập tức** sau khi hoạt động hiện tại kết thúc, trước khi Event Loop tiếp tục chạy qua các pha khác. Nếu gọi đệ quy `nextTick()`, bạn sẽ làm nghẽn (starve) Event Loop, khiến các I/O khác bị đóng băng hoàn toàn.
> *   `setImmediate()` được thiết kế để xếp lịch chạy ở pha **Check Phase** của vòng lặp Event Loop tiếp theo, đảm bảo không làm nghẽn các pha xử lý khác.
...
> [!TIP]
> **Tại sao Streams tránh được lỗi OutOfMemory?**
> *   Khi không dùng Streams, toàn bộ dữ liệu phải được tải lên bộ nhớ RAM trước khi ghi hoặc gửi đi. Điều này tiêu hao RAM tỷ lệ thuận với kích thước file.
> *   Khi dùng Streams, dữ liệu được "băm nhỏ" thành các Buffer Chunk (thông thường mặc định khoảng 64KB). Node.js chỉ lưu trữ tạm thời một vài Chunk trong RAM tại một thời điểm, chuyển tiếp chúng sang đích, và giải phóng bộ nhớ ngay lập tức để nạp Chunk tiếp theo. Nhờ đó, việc truyền tải một file 10GB chỉ tốn khoảng vài chục Megabytes RAM.
